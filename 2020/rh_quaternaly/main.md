...
protected -- for example by a lock or using some language primitives to make
the accesses atomic.
...
software?
We believe heavy-duty program analysis is one option which can offer some help.
...
work correctly.
For example, consider an insertion to a doubly-linked list.
The thread that performs the insertion will create a new node, set its value,
and set the pointers to the previous and next node inside the node-to-be-added.
...
miss some of inserted elements when iterating over it, or even find different
...
never executed at the same time during our testing.
However, this does not mean they cannot be executed at the same time.
...
They range from relatively fast code analysis techniques similar to compiler
warnings, going to more involved techniques like various thread sanitisers that
can detect unsynchronised access to the same memory region from multiple
threads, and finally to heavy-duty analyses that essentially explore all
possible executions of the parallel program.
...
Automated heavy-duty tools do this using many different basic techniques (e.g.,
stateless model checking, explicit-state model checking, symbolic model
checking, symbolic execution), but in essence, they explore all the
possibilities for the timing of threads that can lead to different outcomes.
...
threads and input data can influence run of the test.
...
example, it triggers growth in a test of a thread-safe hashset).
...
While Meltdown and Spectre affect the security of programs and operating
...
Intel and AMD.
These processors power most of the modern laptops, desktops, and servers and
other processors, e.g., high-performance ARM, are often even more relaxed.
...
Research in this area is much less intensive than in the case of relaxed
memory.
Most approaches to termination focus on sequential programs, or on specialised
...
Some resource sections can be marked automatically by DIVINE.
