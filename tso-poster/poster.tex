...
In this evaluation, we compare our approach with state-of-the-art bounded model checker CBMC and stateless model checker Nidhugg and show that extended \divine competes well with both of them.
...
The following table shows how many benchmarks were completed by each of the tools, how many of these contained errors under \xtso, but not without relaxed memory, and how many benchmarks were only solved by given tool and not the other two tools.
...
\begin{tikzpicture}[ ->, >=stealth', shorten >=1pt, auto, node distance=3cm
  ...
  \node () [anchor=center] at (-8.5,-3.5) {\color{blue}\texttt{y $\leftarrow$ 2}};

  \draw [-] (-6,0) rectangle (-3,-2);
  \draw [-] (-6,-1) -- (-3,-1);
...
\end{columns}
\bigskip
Suppose \texttt{thread0} is about to execute with the displayed contents of
store buffers of other two threads and suppose it will load value 2 from
\texttt{y}.
...
\begin{tikzpicture}[ ->, >=stealth', shorten >=1pt, auto, node distance=3cm
  ...
  \draw [-] (-10,-1) -- (-7,-1)
            ...
            (-10,-3) -- (-7,-3);
  \node () [anchor=center] at (-8.5, 0.5) {s.b. 1};
  \node () [anchor=center] at (-8.5,-0.5) {\color{red}\texttt{x $\leftarrow$ 1}};
  \node () [anchor=center] at (-8.5,-1.5) {\color{red}\texttt{y $\leftarrow$ 1}};
  \node () [anchor=center] at (-8.5,-2.5) {\color{red}\texttt{x $\leftarrow$ 2}};
  \node () [anchor=center] at (-8.5,-3.5) {};

  \draw [-] (-6,0) rectangle (-3,-2);
  \draw [-] (-6,-1) -- (-3,-1);
  \node () [anchor=center] at (-4.5, 0.5) {s.b. 2};
  \node () [anchor=center] at (-4.5,-0.5) {\color{blue}\texttt{x $\leftarrow$ 3}};
  \node () [anchor=center] at (-4.5,-1.5) {\texttt{y $\leftarrow$ 3}};
...
\begingroup
    \tt
    \textcolor{dgreen}{void} thread0() \{ \\
    \ \ \ \ \textcolor{dgreen}{int} a = y; \textcolor{gray}{// \textrightarrow 2} \\
    \ \ \ \ \textcolor{dgreen}{int} b = x; \\
    \}
\endgroup
\end{column}
...
At this point, entries of store buffer 1 are marked as flushed. If \texttt{x} is
now loaded from store buffer 2, both \texttt{x} entries from store buffer 1
will be removed.
